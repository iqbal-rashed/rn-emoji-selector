...
export const emojiFromUtf16 = (utf16: string) =>
...
const EmojiPicker = ({
  ...
  const [category, setCategory] = useState<Category>(
    finalCategories.find((c) => c.key === defaultCategory) || finalCategories[1]
  ); // smiley

  const { sections } = useMemo(() => {
    // expensive calc @todo speed up
    const emojiList: Record<
      string,
      {
        key: string;
        category: string;
    ...
    > = {}; // map of emojis to categories
    finalCategories.forEach((category) => {
      const key: string = category.key;
      const list =
        key === RECENT
          ? recent
          : sortEmojis(emojiByCategory(category.name, emojis));
      emojiList[key] = chunkEmojis(list, key, perLine);
    });
    const sections = finalCategories.map((category) => ({
      name: category.name,
      key: category.key,
    ...
    return [
      {
        name: "Search results",
        key: SEARCH,
        data: chunkEmojis(filtered, SEARCH, perLine),
      },
    ]; // create one section on results
  }, [emojis, searchQuery]);
